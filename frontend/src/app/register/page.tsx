...
// Update the schema to remove rememberMe
...
export default function Register() {
    ...
    // Use useForm with the zod resolver
    const { register, handleSubmit, formState: { errors } } = useForm<FormValues>({
        resolver: zodResolver(schema),
    });

    // Define the onSubmit handler with correct type
                            ...
                            <div className="border border-gray-300 rounded-lg p-6 max-w-md shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)] max-md:mx-auto">
                                <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
                                    ...
                                    <div>
                                        ...
                                        <div className="relative flex items-center">
                                            <input
                                                {...register('username')}
                                                type="text"
                                                className={`w-full text-sm text-gray-800 border border-gray-300 px-4 py-3 rounded-lg outline-blue-600 ${errors.username ? 'border-red-500' : ''}`}
                                                placeholder="Enter username"
                                            />
                                            {errors.username && (
                                                <span className="absolute right-4 text-red-500 text-xs">{errors.username.message}</span>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <p className="text-sm !mt-8 text-center text-gray-800">
                                         Already have an account ?<a href="/login" className="text-blue-600 font-semibold hover:underline ml-1 whitespace-nowrap">Login here</a>
                                    </p>
                                </form>
                            </div>
